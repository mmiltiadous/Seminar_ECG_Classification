...
#change verbose to 1
#comment:prints best test metrics of the 5 iterations
class Classifier_NNE:

    def create_classifier(self, model_name, input_shape, nb_classes, output_directory, verbose=1,
                          build=True):
    ...
    def __init__(self, output_directory, input_shape, nb_classes, verbose=1, nb_iterations=5,
                 clf_name='inception'):
    ...
    def fit(self, x_train, y_train, x_val, y_val, x_test, y_test, y_true):
        ...
        df_metrics2.to_csv(self.output_directory + 'df_accuracies.csv', index=False) #change

        full_path = self.output_directory + 'classification_report.csv'
