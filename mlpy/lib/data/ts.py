...
def normalize(X, mode, **kwargs):
    ...
    different from the common spreadsheet datasets and image datasets. Therefore, it is dose not has the
    ...
    # of features and preserving zero entries in sparse datasets.
    ...
    # Scaling datasets with outliers:
    elif mode == 'scale-robust':
        ret = skpre.robust_scale(X, axis=axis, **kwargs)
    ## Non-linear transformation
    # Quantile transforms maps datasets to a uniform distribution. It should be noted that such
    ...
    # Power transforms aim o map datasets from any distribution to as close to a Gaussian
...
def down_sample_batch(x, span_base, step_size, n_step):
    if x.shape[1] == 26: # the case for datasets JapaneseVowels MTS
